...
| 게시글 삭제 시 이미지 해제 | 게시글 삭제(소프트/영구) 시 연결된 이미지를 orphan으로 자동 전환 |
| Soft-delete 영구 삭제 | 소프트 삭제 후 7일 경과 시 파일 + DB 레코드 영구 삭제 |
| 관리 API | Orphan 현황 조회, 목록 조회 및 강제 정리 실행 엔드포인트 제공 |
| 관리자 페이지 | 고아 이미지 통계, 목록 확인, 개별 삭제 및 강제 일괄 정리 UI (TTL 무시, 로그인 시 Header에 Admin 링크 표시) |
...
| `POST` | `/api/upload/admin/cleanup` | Orphan 이미지 강제 정리 실행 (TTL 무시) | 필요 |
...
- **이미지 연결**: 포스트 마크다운 내 이미지 URL 파싱으로 자동 연결/해제, 게시글 삭제 시 orphan 전환
- **이미지 정리**: 미연결 임시 이미지(24시간) 및 소프트 삭제 이미지(7일) 자동 정리, 관리자 강제 정리(TTL 무시) 지원
